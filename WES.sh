...
# Programs needed: FastQC, Trimmomatic, BWA, GATK, ANNOVAR, samtools
...
# Step 2: Data Preprocessing with Trimmomatic for adapter removal and quality trimming
trimmomatic PE \
  "$raw_fastq_dir/sample_R1.fastq.gz" \
  "$raw_fastq_dir/sample_R2.fastq.gz" \
  "$output_dir/sample_R1_paired.fastq.gz" \
  "$output_dir/sample_R1_unpaired.fastq.gz" \
  "$output_dir/sample_R2_paired.fastq.gz" \
  "$output_dir/sample_R2_unpaired.fastq.gz" \
  ILLUMINACLIP:"$adapter_sequences":2:30:10 \
...
# Adjust the column number ('$10') based on the actual column in your output that contains allele frequency data.